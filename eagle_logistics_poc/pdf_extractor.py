  ...
  with pdfplumber.open(str(pdf_path)) as pdf:
    for page_idx, page in enumerate(pdf.pages):
      ...
      for t_idx, table in enumerate(detected[:max_tables_per_page]):
        ...
        tables_out.append({"page_number": page_number, "table_index": t_idx, "rows": rows})
...
def pdf_pages_as_png_bytes(pdf_path: str, *, max_pages: int = 8, scale: float = 2.0) -> List[bytes]:
  path = Path(pdf_path)
