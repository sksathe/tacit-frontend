...
import { buildTranslatedManualPdfLib, buildTranslationQAPdfLib } from "@/lib/manuUnicodePdfExport";
...
export function downloadTranslationQAPdf(run: ManuRun): Promise<void> {
  return buildTranslationQAPdfLib(run);
}

export function downloadTranslatedManualPdf(run: ManuRun, langCode: string): Promise<void> {
  return buildTranslatedManualPdfLib(run, langCode);
}
...
export function downloadManuExportPdf(kind: ManuExportKind, run: ManuRun): void | Promise<void> {
  switch (kind) {
    ...
    case "regulatory_checklist":
      ...
      return;
    case "translation_qa":
      return downloadTranslationQAPdf(run);
    case "audit_package":
